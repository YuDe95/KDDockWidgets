...
public:
    ...
    virtual void show() { Q_ASSERT(false); };
    virtual void hide() = 0;
    ...
    virtual QWindow* tlwWindow() const = 0;
    virtual void onResized(QSize newSize);
    virtual void onLayoutRequested() {};
...
To widget_cast(Widget *widget)
{
    if (!widget)
        return nullptr;

    return dynamic_cast<To>(widget->asQObject());
}

template <typename To>
To widget_cast(const Widget *widget)
{
    if (!widget)
        return nullptr;
